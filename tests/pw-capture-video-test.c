...
struct stream_data {
    ...
    struct timespec last_frame_ts;      // время последнего кадра
    double avg_fps;                     // скользящий средний FPS
};
...
static void on_process(void *userdata) {
    ...
    if (stream_data->last_frame_ts.tv_sec != 0 ||
        stream_data->last_frame_ts.tv_nsec != 0) {
        long sec = now.tv_sec - stream_data->last_frame_ts.tv_sec;
        long nsec = now.tv_nsec - stream_data->last_frame_ts.tv_nsec;
        double delta = (double)sec + (double)nsec / 1e9;
        if (delta > 0) {
            double inst_fps = 1.0 / delta;
            if (stream_data->avg_fps == 0.0)
                stream_data->avg_fps = inst_fps;
            else
                stream_data->avg_fps =
                    stream_data->avg_fps * 0.9 + inst_fps * 0.1;
        }
    }
    stream_data->last_frame_ts = now;
#ifdef FPS_COUNTER
    if (stream_data->frame_count % 30 == 0) {
        printf("  Avg FPS: %.2f\n", stream_data->avg_fps);
    }
#endif
}
