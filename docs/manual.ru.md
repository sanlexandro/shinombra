[![Language: English](https://img.shields.io/badge/Lang-English-blue.svg)](./manual.md)
...
Необходимо измерить и вписать 4 значения:
 ...
 - `led_length` - размер одного блока светодиода именно между местами, в которых можно произвести разрез (что такое блок светодиодов можно прочитать в начале [второго шага](#шаг-2-размеры-монитора-положение-ленты-и-зона-чтения));
...
Дальнейшие главы нужны для более точной настройки ленты и понимания внутренних процессов. Если Вам уже не терпится попробовать `shinombra` в деле, рекомендую использовать [пресеты](./presets.ru.md).
