...
- [Шаг 2. Размеры монитора и положение ленты](#шаг-2-размеры-монитора-и-положение-ленты)
...
## Шаг 2. Размеры монитора и положение ленты
...
![Пример блока светодиодов](./manual/led_block_example.png)
...
![Пример снятия размеров монитора](./manual/frame_sizes_example.drawio.svg)
...
![Пример замера зазора gap](./manual/gap_example.drawio.svg)
...
После замера положения ленты на мониторе можно переходить к следующему шагу.
...
![Пример подключения ленты](./manual/led_connection_example.svg)
...
Дальнейшие главы нужны для более точной настройки ленты и понимания внутренних процессов. Если Вам уже не терпится попробовать `shinombra` в деле, рекомендую использовать пресеты <span style="color:red">НАДА ДОПИСАТЬ</span>.
...
![Принцип работы Checkerboard](./manual/checkerboard_processor.drawio.svg)
...
![Принцип работы CrawlCheckerboard](./manual/crawl_checkerboard_processor.drawio.svg)
...
![Пример с рыбками](./manual/fish_example.jpg)
![Расчёт примера с рыбками](./manual/average_fish_example.svg)
![Пример с котиком](./manual/curly_cat_example.jpg)
![Расчёт примера с котиком](./manual/average_curly_cat_example.svg)
...
Ниже представлены примеры работы на различных изображениях. Первым идёт исходное изображение, а затем за ним идёт то же полупрозрачное изображение с наложенными значениями блоков светодиодов (без фильтров): 
![Пример с рыбками](./manual/fish_example.jpg)
![Расчёт примера с рыбками](./manual/histogram_fish_example.svg)
![Пример с котиком](./manual/curly_cat_example.jpg)
![Расчёт примера с котиком](./manual/histogram_curly_cat_example.svg)
...
![Пример влияния ema](./manual/ema.svg)
...
Gamma - это стандартный фильтр, призванный корректировать изменения яркости, согласно строению человеческого глаза. В результате эволюции человек научился различать разницу в тусклом цвете сильнее, чем в ярком, т.е. изменение яркости ($[0; 255]$) $0 \rarr 1$ будет заметно сильнее, чем $254 \rarr 255$, поэтому gamma превращает результаты анализа кадра в приятные для человека цвета, используя степенную функцию (для нормированной яркости. Т.е. диапазон $[0; 1]$): 
...
$$\begin{cases}  
    V_{out} = 0, \quad S_{out} = 0 & \text{если } V_{in} < V_{threshold} \quad \text{(полное отсечение)} \\ 
    
    \begin{aligned}
      V_{out} &= V_{in} \cdot \Big( k(V_{in}) \Big)^{falloff\_exponent} \\  
      S_{out} &= S_{in} \cdot k(V_{in})  \end{aligned} & \text{если } V_{threshold} \le V_{in} < V_{threshold} + V_{fade\_range} \quad \text{(плавная зона)} \\ 
    
    V_{out} = V_{in}, \quad S_{out} = S_{in} & \text{если } V_{in} \ge V_{threshold} + V_{fade\_range} \quad \text{(без изменений)} 
  
  \end{cases} \\

  V_{threshold} = {{threshold \cdot 255} \over {100}} \quad \text{(перевод из процентов)} \\
  V_{fade\_range} = {{fade\_range \cdot 255} \over {100}} \quad \text{(перевод из процентов)} \\

  k = {{V_{in} - V_{threshold}} \over V_{fade\_range}}
$$
...
Настройки данного фильтра хранятся в `[black_threshold_config]`. Необходимо указать 3 параметра:
 ...
 - `falloff_exponent` - "крутизна" кривой, работающей в диапазоне (в диапазоне $[0; \infin)$ математически, но смысл имеет при значениях больше 1).

\* В формулах, конечно, `threshold` и `fade_range` переводятся в соответствующий диапазон.

![Принцип работы параметров](./manual/black_threshold_params.svg)

Ниже представлены графики воздействия при `threshold = 10%`, `fade_range = 10%`, `falloff_exponent = 1.5`:
![Пример влияния black_threshold на яркость](./manual/black_threshold_value.svg)
![Пример влияния black_threshold на насыщенность](./manual/black_threshold_saturation.svg)
...
$$
  \begin{cases}
    ...
    S_{out} = ({{S_{in} - S_{floor}} \over {1 - S_{floor}}})^{boost\_exponent} & \text{если } S_{in} \ge S_{floor} \quad \text{(плавное увеличение)} \\
  \end{cases} \\

  S_{floor} = {{floor} \over {100}} \quad \text{(перевод из процентов)}
$$
...
Настройки данного фильтра хранятся в таблице `[saturation_boost_config]` и требуют указания 2-ух параметров:
 ...
 - `boost_exponent` - "крутизна" кривой, по которой увеличивается насыщенность (в диапазоне $[1; \infin)$).

Ниже представлен график воздействия при `floor = 10%`, `boost_exponent = 1.5`:
![Пример влияния saturation_boost на насыщенность](./manual/saturation_boost.svg)
...
$$
  \begin{cases}
    ...
    V_{out} = V_{prev} + {{\alpha} \over {1 + ({{|D|} \over {S}})^2}} \cdot D \quad \text{(результат)} \\
  \end{cases} \\

  S = {{sensitivity \cdot 255} \over {100}} \quad \text{(перевод из процентов)}
$$
...
![Пример воздействия flash_guard на яркость](./manual/flash_guard.svg)
...
![Пример воздействия Median](./manual/median.svg)
...
Было решено не делать проверочных сумм и прочего, т.к. вероятность помехи в коротком проводе на Serial протоколе $\rarr 0$.
